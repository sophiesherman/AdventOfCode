...
const getSolution = (data) => {
  ...
  const totalFullSpace = directoryTotals['/']
  const unusedSpaceNeededForUpdate = 30000000
  const currentFreeSpace = totalSpace - totalFullSpace
  const spaceToRemove = unusedSpaceNeededForUpdate - currentFreeSpace

  const potentialFilesToRemove = Object.values(directoryTotals)
    .filter((size) => size >= spaceToRemove)
    .sort((a, b) => a - b)

  return potentialFilesToRemove[0]
}
